...
class JiraClient:
    ...
    def __init__(self, config: JiraConfig | None = None) -> None:
        ...
        # Configure connection timeouts (for Cloud Run and network issues)
        # Increase timeouts to handle slow connections
        import requests
        from requests.adapters import HTTPAdapter
        from urllib3.util.retry import Retry
        
        # Create adapter with increased timeouts
        adapter = HTTPAdapter(
            max_retries=Retry(
                total=3,
                backoff_factor=1,
        ...
        # connect timeout: time to establish connection (increased from default 75s)
        # read timeout: time to read response (increased for large responses)
        self.jira._session.mount("http://", adapter)
        self.jira._session.mount("https://", adapter)
        
        # Configure default timeout for all requests (connect, read)
        # This will be used if timeout is not specified in individual requests
        # Format: (connect_timeout, read_timeout)
        # Increased connect timeout to 120s for Cloud Run network issues
        self.jira._session.timeout = (120, 300)  # (connect, read) in seconds
        
        logger.debug("Jira client timeouts configured: connect=120s, read=300s")
