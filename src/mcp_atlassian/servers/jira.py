...
async def search(
    ...
    limit: Annotated[
        int,
        Field(description="Maximum number of results (1-50). MCP server supports up to 50 results per query.", default=10, ge=1, le=50),
    ] = 10,
    start_at: Annotated[
        int,
...
async def get_sprints_from_board(
    ...
    board_id: Annotated[
        str | None,
        Field(description="(Optional) The id of board (e.g., '1000'). If not provided, uses the first available board."),
    ] = None,
    state: Annotated[
        str | None,
    ...
    Args:
        ctx: The FastMCP context.
        board_id: (Optional) The ID of the board. If not provided, uses the first available board.
    ...
    # If board_id is not provided, get the first available board
    if not board_id:
        boards = jira.get_all_agile_boards_model(limit=1)
        if not boards:
            return json.dumps(
                {"error": "No boards found. Please specify a board_id."},
                indent=2,
                ensure_ascii=False
            )
        board_id = boards[0].id
        logger.info(f"get_sprints_from_board: No board_id provided, using first available board: {board_id}")
